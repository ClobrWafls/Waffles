...
import net.mcreator.waffles.block.BungaloLogBlock;
import net.mcreator.waffles.block.BungaloLeavesBlock;
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0f).depth(0.1f).scale(0.2f).temperature(0.7999999999999999f).precipitation(Biome.RainType.NONE)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(Blocks.CAKE.getDefaultState(),
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.CAKE.getDefaultState().getBlock() || ground == Blocks.GRASS_BLOCK.getDefaultState().getBlock())
							&& (ground2 == Blocks.CAKE.getDefaultState().getBlock() || ground2 == Blocks.GRASS_BLOCK.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), Blocks.GRASS_BLOCK.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BungaloLogBlock.block.getDefaultState().getBlock()
					|| blockType == BungaloLeavesBlock.block.getDefaultState().getBlock() || blockType == Blocks.CAKE.getDefaultState().getBlock()
					|| blockType == Blocks.GRASS_BLOCK.getDefaultState().getBlock();
		}
